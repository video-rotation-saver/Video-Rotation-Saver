# install.ps1 — PotPlayerRotate setup
#
...
#   4. Prints a smoke-test command and the default hotkey.
...
if (-not $ff -or -not $ffp) {
    ...
    Write-Host "You can still proceed — edit %APPDATA%\PotPlayerRotate\config.ini with full paths later."
} else {
...
$lnk = Join-Path $startup 'PotPlayerRotate.lnk'
...
if ($ans -eq '' -or $ans -match '^[yY]') {
    ...
    $shortcut.Description = 'PotPlayerRotate tray daemon'
    $shortcut.Save()
...
Write-Host ""
Write-Host "Default hotkey: Ctrl+Shift+R  (edit in %APPDATA%\PotPlayerRotate\config.ini)"
Write-Host ""
